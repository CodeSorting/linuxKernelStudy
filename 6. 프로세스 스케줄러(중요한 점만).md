...
Utilization Clamping(uclamp)은 태스크가 만들어내는 PELT 사용률(util_avg) 신호에 하한과 상한을 씌워, 스케줄러와 cpufreq 거버너에게 "이 태스크에 필요한 성능 범위"를 힌트로 전달하는 메커니즘입니다. Linux 5.3에서 도입되었으며(`CONFIG_UCLAMP_TASK`), 모바일·이기종 SoC의 전력/성능 균형 조정에서 핵심 역할을 합니다. 과거 안드로이드가 비공식 패치(SchedTune)로 해결하던 문제를 메인라인 표준으로 흡수한 결과입니다.
...
### -1 리셋의 비대칭성
sched_util_min/max에 -1을 넣으면 "수동으로 1024를 지정"하는 것과 다릅니다. -1은 시스템 기본값을 따른다는 의미라서, sysctl로 기본값이 바뀌면 함께 변합니다. 또한 SCHED_FIFO/SCHED_RR(RT) 태스크의 기본값은 UCLAMP_MIN=UCLAMP_MAX=1024로 설정되어 과거 RT 동작(항상 최대 주파수)을 보존합니다. 배터리 기반 시스템에서는 kernel.sched_util_clamp_min_rt_default sysctl로 이 RT 기본 하한을 낮춰 전력을 아낄 수 있습니다. 
