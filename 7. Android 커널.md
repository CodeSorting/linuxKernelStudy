...
메모리에 접근할 때 CPU가 일일이 개입하면 너무 느립니다. 그래서 최신 하드웨어 장치(카메라, GPU 등)들은 CPU를 거치지 않고 직접 RAM에 데이터를 읽고 쓸 수 있는 컨트롤러(DMA)를 가지고 있습니다.

fd는 오직 운영체제 커널이 관리하는 '실제 물리 메모리(Physical Memory) 객체'의 신분증명서 역할을 하기 때문에 fd만 받아서 여러곳에서 처리가 가능하다. fd는 프로세스가 가진 `파일 객체를 찾기 위한 번호(인덱스)`이다.

프로세스마다 밑의 그림처럼 fd table이 존재한다.
...
1. /dev/binder라는 디바이스를 찾음
2. struct file 생성
3. 프로세스의 fd table에 등록
4. 사용자에게 번호 반환

fd가 5일 경우 사용자 입장에서는 5번 파일을 달라는 것, 여기에 `struct file 객체`가 있고 이 객체는 DMA-BUF의 핵심 구조체인 `struct dma_buf`와 연결되어 있다.
...
Android 8.0(Oreo)에서 도입된 Project Treble은 `Android Framework`과 `하드웨어 벤더 구현`을 명확히 `분리`합니다. 이 아키텍처 덕분에 Google이 Framework을 업데이트하더라도 벤더 HAL과 커널 드라이버를 수정할 필요가 없습니다.

- VNDK (Vendor NDK): 구글(Framework)이 하드웨어 제조사(Vendor)에게 "이 라이브러리들은 OS가 업데이트되어도 구조를 안 바꿀 테니 안심하고 쓰세요"라고 제공하는 안정적인 도구 상자입니다.
- VINTF (Vendor Interface): OS와 하드웨어가 서로 호환되는지 확인하는 일종의 '계약서'입니다. OS가 켜질 때 명세서를 확인하여 OS 버전과 하드웨어 드라이버 버전이 서로 맞물려 돌아갈 수 있는지 검사합니다.
- /dev/hwbinder: 완전히 분리된 Framework와 하드웨어(HAL)가 서로 데이터를 빠르게 주고받을 수 있도록 뚫어 놓은 전용 통신 파이프(IPC)입니다.

## HIDL → AIDL 전환
Android 13부터 새로운 HAL은 `반드시 Stable AIDL을 사용`해야 합니다. HIDL(HAL Interface Definition Language)은 `deprecated` 상태이며, 기존 HIDL HAL도 점진적으로 AIDL로 마이그레이션 중입니다.
...
## Android init 프로세스
Android의 PID 1 프로세스인 `init`은 일반 Linux의 `systemd/SysVinit`과 다르게 .rc 파일(Android Init Language, 독자적인 초기화 언어)을 파싱하여 서비스를 시작합니다. `Property 시스템(__system_property_*)`은 공유 메모리 기반으로, `/dev/__properties__`를 통해 전 프로세스에 읽기 전용(`시스템 안정성, 보안` 때문)으로 노출됩니다.

* Property (프로퍼티) 시스템: 안드로이드 시스템 전반의 `환경 설정값(예: 기기 모델명, 부팅 상태, 네트워크 설정 등)을 저장하고 관리하는 공간`
...
### fork() + CoW 메커니즘 동작 원리
일반적인 프로세스를 처음부터 시작하면 `ART 런타임`과 `수천 개의 프레임워크 클래스`를 매번 초기화해야 합니다. Zygote는 이 비용을 `한 번만 지불`하고, 이후 모든 앱에 공유합니다.
...
Zygote는 메모리 낭비를 막기 위해 보통 최소 1개에서 최대 3개의 USAP(Unspecialized App Process)를 미리 생성해 둡니다. 무한정 많이 만들어 두는 것이 아니라, `아주 작은 규모의 예비 풀(Pool)`을 유지하는 방식입니다.

안드로이드 내부 `ZygoteConfig.java`에 정의된 구체적인 기본 수치는 다음과 같습니다.

- 최대 유지 개수 (USAP_POOL_SIZE_MAX_DEFAULT): 3개.
- 최소 유지 개수 (USAP_POOL_SIZE_MIN_DEFAULT): 1개.
- 풀 보충 기준점 (USAP_POOL_REFILL_THRESHOLD_DEFAULT): 1개.
- 최대 허용 한도 (USAP_POOL_SIZE_MAX_LIMIT): 100개.
...
### Android cgroup 구성
`cgroup v2 (Android 12+)`: 한정된 기기 자원(CPU, 메모리, 배터리 등)을 여러 앱이 낭비 없이 효율적으로 나눠 쓰도록 엄격하게 통제하기 위해서 나온 개념입니다. 각 앱을 그룹 안의 한 요소로 봅니다. 

Android 12부터 `cgroup v2`가 필수입니다. `task_profiles.json`이 cgroup 설정을 추상화하며, init이 .rc 파일의 `task_profiles` 지시어를 통해 서비스를 적절한 cgroup에 배치합니다.

![alt text](image/image-119.png)

cpuctl (CPU 할당): 현재 화면에 켜져 있는 앱(Foreground)에는 CPU 시간을 많이 주고, 뒤로 내려간 앱(Background)에는 적게 주어 화면 버벅임을 막습니다.

cpuset (코어 할당): 스마트폰 CPU는 보통 고성능 코어(빅 코어)와 저전력 코어(리틀 코어)로 나뉩니다. 무거운 작업은 빅 코어에, 가벼운 백그라운드 작업은 리틀 코어에 물리적으로 묶어두는(Pinning) 역할을 합니다.

memory (메모리 제한): 앱이 사용할 수 있는 최대 메모리를 제한합니다. 기기 메모리가 부족해지면 안드로이드의 lmkd(Low Memory Killer Daemon)가 개입하여 덜 중요한 앱을 강제로 종료시킵니다.

freezer (백그라운드 동결): 앱이 백그라운드로 넘어가면 CPU를 아예 사용하지 못하도록 얼려버려(Freeze) 배터리 소모를 극단적으로 줄입니다.

io (입출력 제한): 앱이 저장소(플래시 메모리)를 읽고 쓰는 속도를 제한하여, 특정 앱이 저장소를 독점해 시스템 전체가 느려지는 것을 방지합니다.

Foreground 앱 (사용 중인 앱)
- cpus: 0-7: 기기의 모든 코어(예: 8코어 전체)를 사용할 수 있게 허락합니다.
- cpu.weight: 1024: CPU를 배분받을 때 매우 높은 우선순위(가중치)를 가집니다.

Background 앱 (뒤로 간 앱)
- cpus: 0-3: 성능이 낮은 저전력 코어(0~3번)만 사용하도록 제한합니다.
- cpu.weight: 50: CPU 우선순위를 대폭 낮춰, 사용 중인 앱에 자원을 양보하게 만듭니다.

UCLAMP (성능 보장 메커니즘)
- cpu.uclamp.min: 20: 사용 중인 앱(Foreground)이 아무리 가벼운 작업을 하더라도, 시스템 스케줄러에게 "최소 20% 이상의 성능은 무조건 할당해 줘"라고 힌트를 줍니다. 이는 갑작스러운 화면 전환이나 스크롤 시 앱이 버벅거리는 것을 막기 위한 성능 보장 장치입니다.
...
# Thermal 관리
Android 기기의 `Thermal HAL`과 커널 `thermal framework`이 협력하여 발열을 제어합니다. 커널이 온도 센서를 모니터링하고 쓰로틀링 정책을 실행하며, Thermal HAL은 유저스페이스 수준의 추가 제어(프레임 레이트 감소, 밝기 조절 등)를 담당합니다. 
```c
